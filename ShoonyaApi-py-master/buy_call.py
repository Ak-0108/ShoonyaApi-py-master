...
def Main(): 
    ...
    i=0
    while i<5:     
      function_buy(nifty_ltp,expiry_month)
      
    print("Time's up!")
    
     
def function_buy(nifty_ltp, expiry_month):     
            ce_strike=dom_strike.calculate_atm_strike(nifty_ltp)
            ce_name=dom_strike.get_ce_name(expiry_month,ce_strike) 

            #above is only once in a day cycle                       
            ce_buy_price=dom_strike.get_nifty_ce_rate(ce_strike,expiry_month) #obj1.read_ce_pe_rate(pe_name)
            print(f'Buy Call '+ ce_name  +' at: ' +str(ce_buy_price))
            
            v_buy_done= csv_manager.read_single_field('db/ce_order.csv')
            if len(v_buy_done)==0:
            #Order submission
                submit_buy_order(ce_pe_name=ce_name,buy_price=ce_buy_price)
                csv_data=[ce_name]
                csv_manager.write_single_field(csv_data,'db/ce_order.csv')
...
def submit_buy_order(ce_pe_name,buy_price):
    ...
    activeOrderId=data_insert.place_NSE_Option_Order(ce_pe_name,buy_price,quanity_nifty) 
...
def sale_order(cepe_name):
     ...
     if len(lstPosition)>0:
          ...
          for item in lstPosition:
            ...
            if balance_qty>0:  
                v_buy_done= csv_manager.read_single_field('db/sale_cepe_order.csv')
                if len(v_buy_done)==0:
                    if checkSaleOrder(item.name)==True: 
                        obj_sale= order1.MyClass()
                        activeOrderId=obj_sale.place_NSE_Option_Order_Sell(cepe_name,item.daybuyavgprc+5,item.daybuyqty)
                        print(item.name+" Sale Order done!")
                        csv_data=[item.name]
                        csv_manager.write_single_field(csv_data,'db/sale_cepe_order.csv')

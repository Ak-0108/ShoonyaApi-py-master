...
def function_buy(nifty_ltp, expiry_month):     
            ...
            v_buy_done= csv_manager.read_single_field('db/ce_order_market.csv')
            if len(v_buy_done)==0:
            #Order submission
                submit_buy_order(ce_pe_name=ce_name)
                csv_data=[ce_name]
                csv_manager.write_single_field(csv_data,'db/ce_order_market.csv')
...
def submit_buy_order(ce_pe_name):
    ...
    activeOrderId=data_insert.place_NSE_Option_Market(ce_pe_name,quanity_nifty) 
...
def sale_order(cepe_name):
     ...
     if len(lstPosition)>0:
          ...
          for item in lstPosition:
            ...
            if balance_qty>0:  
                v_buy_done= csv_manager.read_single_field('db/sale_ce_order_market.csv')
                if len(v_buy_done)==0:
                    if checkSaleOrder(item.name)==True: 
                        ...
                        csv_data=[item.name]
                        csv_manager.write_single_field(csv_data,'db/sale_ce_order_market.csv')
